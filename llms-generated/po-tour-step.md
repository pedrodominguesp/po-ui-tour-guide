...
**Referência:** https://po-ui.io/documentation/po-tour-step

Interface que descreve um passo individual do tour guiado executado pelo `PoTourService`.
...
|---|---|---|---|
| `align` | `PoTourAlignment` | sim | Alinhamento do popover ao longo do eixo da posição configurada em `position`. |
...
| `onBeforeHighlight` | `(step: PoTourStep, index: number) => void` | sim | Hook* executado imediatamente antes de o passo ser destacado na página, sincronamente à transição. |
| `onDeselected` | `(step: PoTourStep, index: number) => void` | sim | Hook* executado quando o passo deixa de estar ativo, seja por avanço, retrocesso ou encerramento do tour. |
| `onHighlighted` | `(step: PoTourStep, index: number) => void` | sim | Hook* executado imediatamente após o passo ser destacado na página. |
| `position` | `PoTourPosition` | sim | Posição preferida do popover em relação ao elemento destacado. |
