...
**Referência:** https://po-ui.io/documentation/po-tour-guide-step-change-event

Interface que descreve o evento emitido pelo `PoTourGuideService` a cada mudança de passo durante a execução do tour.

É publicado no `Observable` `PoTourGuideService.stepChangeInterface que descreve o evento emitido pelo `PoTourGuideService` a cada mudança de passo durante a execução do tour.
...
programática aos métodos `next`, `previous`, `goTo` e `start` do `PoTourGuideService`.

As aplicações consumidoras podem assinar `stepChangeInterface que descreve o evento emitido pelo `PoTourGuideService` a cada mudança de passo durante a execução do tour.
...
| `step` | `PoTourGuideStep` | não | Passo do tour que se tornou ativo após a transição. |
