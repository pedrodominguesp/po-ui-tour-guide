...
**Referência:** https://po-ui.io/documentation/po-tour-guide-service

Serviço responsável por criar e controlar tours guiados na interface da aplicação.

O `PoTourGuideService` permite apresentar uma sequência de passos visuais para orientar
...
O serviço centraliza a configuração do tour, incluindo:
...
A mesma proteção é aplicada aos *labels* definidos em `PoTourGuideStep` e
`PoTourGuideOptions` quando construídos dinamicamente.
...
É possível alterar a aparência do *popover* renderizado pelo `PoTourGuideService` através dos tokens
(CSS) consumidos pelo arquivo `po-tour-guide.css` distribuído via `@po-ui/style`.
